...
var formatXmlInput = function formatXmlInput(params) {
  ...
  var values;
    ...
    {
      //count++;
      values = params[keys[val]];

      switch (typeof values) {
          case 'object':
          Object.keys(values).forEach(function(key) {
            if (typeof values[key] == 'object') {
              values[key] = formatXmlInput(values[key]);
            };
          ...
          default:
            var result = {};
            result[keys[val]] = params[keys[val]];
      ...
      if (typeof values != 'string' && typeof values != 'number')
      {
          ...
          {            
            Object.keys(values).forEach(function(key) {
              result = params[keys[val]];
              Item.push(result);
            });
          }
          else if (typeof params[keys[val]] == 'object')
          {
            result[keys[val]] = [];
            Object.keys(values).forEach(function(key) {
              var property = {};
              property[key] = values[key];
              result[keys[val]].push(property);
            });
...
var defaultParams = function defaultParams(options) {
  ...
  return {
    ...
    'Shopping': {
      ...
      'X-EBAY-API-VERSION': options.version ? options.version : '931',
